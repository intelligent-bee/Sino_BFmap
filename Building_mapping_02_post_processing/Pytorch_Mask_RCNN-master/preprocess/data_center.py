...
class CocoDataset(data.Dataset):
    ...
    def __init__(self,config, class_ids=None,
                  class_map=None, return_coco=False,argument=True,use_mini_mask = False,transform = None,data_test=None,root='/home/wangziqiao/llx/data/buildings/Shanghai/train_data'):
        self.image_info = []
        self.image_ids=[]
        # Background is always the first class
        self.class_info = [{"source": "", "id": 0, "name": "BG"}]
    ...
    def __getitem__(self, idx):
        ...
        # if sample.shape[1]==9:
        class_id=sample[:,0]-1
        # class_id=np.array([i if i!=19 else 0 for i in class_id],dtype=np.float64)
        boxes=sample[:,1:5]
        ...
        padded_boxes = torch.zeros((self.maxbox, 4))#padding 到 maxbox
        ...
        if self.data_test is not None:
            return image,num_boxes,padded_class, padded_boxes, padded_geo_info,img_id
        else:
