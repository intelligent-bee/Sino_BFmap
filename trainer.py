...
def trainer_dataset(args, model, snapshot_path):
    ...
    db_train = StreamingGeospatialDataset(
        ...
        image_transform=image_transforms, label_transform=label_transforms,nodata_check=None
    ) #
    ...
    Class_weight =torch.FloatTensor([0.2,1,4,5,5,1,2,0.7,6,0]).to(device)
    ce_loss = CrossEntropyLoss(ignore_index=20,weight=Class_weight)
    ...
    for epoch_num in iterator:
        ...
        logging.info('Epoch : %d, CE-branch1 : %f, CE-branch2: %f, loss: %f' % (epoch_num, avg_loss1, avg_loss2, avg_loss1*0.5+avg_loss2*0.5))
        save_interval = 5 
